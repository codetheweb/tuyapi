...
class MessageParser {
  ...
  encode(options) {
    ...
    // Hand off to individual protocols
    if (this.version === '3.3') {
      return this.encode33(options);
    }

    return this.encode31(options);
  }
  ...
  encode33(options) {
    ...
    const extendedHeader = options.commandByte === CommandType.DP_QUERY ?
      0 : 15;

    // Allocate buffer with room for payload + 24 bytes for
    // prefix, sequence, command, length, crc, and suffix
    // and potentially + 15 bytes for 3.3 header
    const buffer = Buffer.alloc(payload.length + 24 + extendedHeader);

    // Add prefix, command, and length
    buffer.writeUInt32BE(0x000055AA, 0);
    buffer.writeUInt32BE(options.commandByte, 8);
    buffer.writeUInt32BE(payload.length + 8 + extendedHeader, 12);
    ...
    // Add 3.3 header
    if (extendedHeader) {
      Buffer.from('3.3').copy(buffer, 16);
      buffer.writeUInt32BE(0, 19);
      buffer.writeUInt32BE(0, 23);
      buffer.writeUInt32BE(0, 27);
    }

    // Add payload, crc, and suffix
    payload.copy(buffer, 16 + extendedHeader);
    const crcSum = crc(buffer.slice(0, -8));
    buffer.writeInt32BE(crcSum, payload.length + 16 + extendedHeader);
    buffer.writeUInt32BE(0x0000AA55, payload.length + 20 + extendedHeader);
