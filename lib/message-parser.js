...
MessageParser.prototype.encode = function (options) {
  ...
  const crc32 = Buffer.from(crc.crc32(Buffer.concat([prefix, payload])).toString(16), 'hex');

  // Add CRC32
  const concatBuffer = Buffer.concat([prefix, payload, crc32, suffix]);
...
module.exports = {parse: parse, encode: encode};
