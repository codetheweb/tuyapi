...
MessageParser.prototype._parse = function () {
  ...
  // Slice off CRC and suffix, put remainder into new const
  const data = this._buff.slice(0, this._buff.length - 8);

  // Get CRC
  const actualCRC = this._buff.readUInt32BE(this._buff.length - 8);

  // Calculate what CRC should be
  const expectedCRC = crc.crc32(data);

  // Check if CRCs match
  if (expectedCRC !== actualCRC) {
    throw new Error('Invalid CRC32 expected: ' + expectedCRC + ' got ' + actualCRC);
  }
...
MessageParser.prototype._encode = function (options) {
  ...
  // Calculate length
  const len = (payload.toString('hex').length + 16) / 2;
  ...
  // Calculate CRC and convert to Buffer
