import { MessageParser, CommandType, type Packet } from './message-parser'
import { UDP_KEY } from './config'
import * as EventEmitter from 'node:events'
import PQueue from '../node_modules/p-queue/dist/index'
import * as pTimeout from '../node_modules/p-timeout/index'
import * as pRetry from '../node_modules/p-retry/index'
import { Socket } from 'node:net'
import * as dgram from 'node:dgram'
import debugFn from 'debug'

const debug = debugFn('TuyAPI')

type TuyaDeviceOptions = {
	/** IP of device */
	ip?: string
	/** Port of device */
	port?: number
	/** ID of device (also called `devId`) */
	id?: string
	/** Gateway ID (not needed for most devices) */
	gwID?: string
	/** Encryption key of device (also called `localKey`) */
	key: string
	/** Product key of device (currently unuse) */
	productKey?: string
	/** Protocol verison */
	version?: number
	/** If true (default), emits a data event containing a payload
	 * of null values for on-device JSON parsing error */
	nullPayloadOnJSONError?: boolean
	/** if true (default), sends GET request after connection is established.
	 * This should probably be `false` in synchronous usage. */
	issueGetOnConnect?: boolean
	/** if true (default), sends DP_REFRESH request after connection is established.
	 * This should probably be `false` in synchronous usage. */
	issueRefreshOnConnect?: boolean
	/** if true (default), sends DP_REFRESH and GET request after every ping.
	 * This should probably be `false` in synchronous usage. */
...
export class TuyaDevice extends EventEmitter {
	...
	readonly device: Pick<
		TuyaDeviceOptions,
		'ip' | 'port' | 'id' | 'gwID' | 'key' | 'productKey'
	> & { version: string; parser: MessageParser }

	readonly globalOptions: Pick<
		TuyaDeviceOptions,
		'issueGetOnConnect' | 'issueRefreshOnConnect' | 'issueRefreshOnPing'
	>

	readonly nullPayloadOnJSONError: boolean

	private foundDevices: unknown[]

	private _connected: boolean
	private _responseTimeout = 2
	private _connectTimeout = 5 // Seconds
	private _pingPongPeriod = 10 // Seconds
	private _lastPingAt = new Date()

	private _currentSequenceN = 0
	private _resolvers = {}
	private _setQueue = new PQueue({ concurrency: 1 })
	...
	private _dpRefreshIds = [4, 5, 6, 18, 19, 20]
	private _tmpLocalKey: Buffer | null = null
	private _tmpRemoteKey: Buffer | null = null
	private sessionKey: Buffer | null = null

	private _expectRefreshResponseForSequenceN: number | undefined = undefined

	private _setResolver: ((data: unknown) => void) | undefined = undefined
	private _setResolveAllowGet: boolean | undefined = undefined

	private _pingPongInterval: NodeJS.Timeout | null = null
	private _pingPongTimeout: NodeJS.Timeout | null = null
	...
	constructor({
		...
		// Check arguments
		if (!(isValidString(id) || isValidString(ip))) {
		...
		this.device = {
			...
			parser: new MessageParser({ key, version }),
		}
	 ...
	 * @param options.schema true to return entire list of properties from device
	 * @param options.dps DPS index to return
	 * @param options.cid if specified, use device id of zigbee gateway and cid of subdevice to get its status
	 * @example
	 ...
	 */
	async get(options: { schema: boolean; dps?: number; cid: string }): Promise<
		boolean | object
	> {
		const payload = {
		...
		const commandByte =
			this.device.version === '3.4' || this.device.version === '3.5'
		...
		if (
			...
			data === 'json obj data unvalid' ||
			data === 'data format error' /* || data === 'devid not found' */
		) {
	 ...
	 * @param options.schema true to return entire list of properties from device
	 * @param options.dps DPS index to return
	 * @param options.cid if specified, use device id of zigbee gateway and cid of subdevice to refresh its status
	 * @param options.requestedDPS only set this if you know what you're doing
	 * @example
	 * // get first, default property from device
			...
			this._send(buffer)
				.then(async (data) => {
					if (data === 'json obj data unvalid') {
						...
						// For schema there's currently no fallback options
						const setOptions = {
							dps: options.requestedDPS
								? options.requestedDPS
								: this._dpRefreshIds,
							set: null,
							isSetCallToGetData: true,
						}
	 ...
	 * @param options Options object
	 * @param options.dps DPS index to set
	 * @param options.set value to set
	 * @param options.cid if specified, use device id of zigbee gateway and cid of subdevice to set its property
	 * @param options.multiple Whether or not multiple properties should be set with options.data
	 * @param options.isSetCallToGetData Wether or not the set command is used to get data
	 * @param options.data Multiple properties to set at once. See above.
	 * @param options.shouldWaitForResponse see
	...
	set(options: {
		...
		options.shouldWaitForResponse =
			typeof options.shouldWaitForResponse === 'undefined'
				? true
				: options.shouldWaitForResponse
		...
		if (!options.isSetCallToGetData) {
			...
			for (const key of dps) {
				options.isSetCallToGetData =
					options.isSetCallToGetData && dps[key] === null
			}
		}
			...
			pTimeout(
				new Promise((resolve, reject) => {
					if (options.shouldWaitForResponse && this._setResolver) {
						throw new Error(
							'A set command is already in progress. Can not issue a second one that also should return a response.',
						)
					}
	 ...
	 * @returns returned data for request
	 */
	private _send(buffer: Buffer): Promise<unknown> {
	 ...
	 */
	createDeferredConnectPromise(): void {
		let res
		let rej
	...
	private _packetHandler(packet: Packet) {
		// Protocol 3.4, 3.5 - Response to Msg 0x03
		if (packet.commandByte === CommandType.SESS_KEY_NEG_RES) {
			if (!this.connectPromise) {
				debug(
					'Protocol 3.4, 3.5: Ignore Key exchange message because no connection in progress.',
				)
				return
			}
			...
			debug(
				`Protocol 3.4, 3.5: Local Random Key: ${this._tmpLocalKey.toString('hex')}`,
			)
			debug(
				`Protocol 3.4, 3.5: Remote Random Key: ${this._tmpRemoteKey.toString('hex')}`,
			)
			...
			const calcLocalHmac = this.device.parser
				.cipher!.hmac(this._tmpLocalKey)
				.toString('hex')
			...
			if (this.device.version === '3.4') {
				this.sessionKey = this.device.parser.cipher._encrypt34({
					data: this.sessionKey,
				})
			} else if (this.device.version === '3.5') {
				this.sessionKey = this.device.parser.cipher._encrypt35({
					data: this.sessionKey,
					iv: this._tmpLocalKey,
				})
			}

			debug(
				`Protocol 3.4, 3.5: Session Key: ${this.sessionKey.toString('hex')}`,
			)
			debug('Protocol 3.4, 3.5: Initialization done')

			this.device.parser.cipher!.sessionKey = this.sessionKey
		...
		if (packet.commandByte === CommandType.DP_REFRESH) {
			...
			if (typeof this._setResolver === 'function') {
				debug(
					'Received DP_REFRESH empty response packet without STATUS packet from set command - resolve',
				)
				...
				this._expectRefreshResponseForSequenceN &&
				this._expectRefreshResponseForSequenceN in this._resolvers
			) {
				...
				debug(
					`Received DP_REFRESH response packet - no resolver found for sequence number${packet.sequenceN}`,
				)
			}
		...
		if (
			...
			packet.payload &&
			packet.payload.dps &&
			typeof packet.payload.dps[1] === 'undefined'
		) {
	...
	disconnect() {
		...
		this._connected = false
		this.device.parser.cipher!.sessionKey = null
	 ...
	 * @param options Options object
	 * @returns Promise that resolves to `true` if device is found, `false` otherwise.
	 */
	resolveId(options: { timeout?: number; all?: boolean }): Promise<
		boolean | unknown[]
	> {
		console.warn(
			'resolveId() is deprecated since v4.0.0. Will be removed in v5.0.0. Use find() instead.',
		)
	 ...
	 */
	find({ timeout = 10, all = false }: { timeout?: number; all?: boolean }) {
		...
		const listenerEncrypted = dgram.createSocket({
			type: 'udp4',
			reuseAddr: true,
		})
	 ...
	 */
	async toggle(property = '1'): Promise<boolean> {
 ...
 * @returns `true` if is string and length != 0, `false` otherwise.
 */
